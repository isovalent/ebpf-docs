...
* **BPF_F_TUNINFO_IPV6**: Indicate that the tunnel is based on IPv6 protocol instead of IPv4.

* **BPF_F_ZERO_CSUM_TX**: For IPv4 packets, add a flag to tunnel metadata indicating that checksum computation should be skipped and checksum set to zeroes.

* **BPF_F_DONT_FRAGMENT**: Add a flag to tunnel metadata indicating that the packet should not be fragmented.

* **BPF_F_SEQ_NUMBER**: Add a flag to tunnel metadata indicating that a sequence number should be added to tunnel header before sending the packet. This flag was added for GRE encapsulation, but might be used with other protocols as well in the future.

* **BPF_F_NO_TUNNEL_KEY**: Add a flag to tunnel metadata indicating that no tunnel key should be set in the resulting tunnel header.
