---
title: "KFunc 'scx_bpf_dsq_insert___v2'"
description: "This page documents the 'scx_bpf_dsq_insert___v2' eBPF kfunc, including its definition, usage, program types that can use it, and examples."
---
# KFunc `scx_bpf_dsq_insert___v2`

<!-- [FEATURE_TAG](scx_bpf_dsq_insert___v2) -->
[:octicons-tag-24: v6.19](https://github.com/torvalds/linux/commit/2dbbdeda77a61b39dc4a34dfce873907cfea2c4b)
<!-- [/FEATURE_TAG] -->

This function inserts a task into the First In First Out(FIFO) queue of a Dispatch Queue(DSQ).

!!! note
    This is the newer version of [`scx_bpf_dsq_insert`](scx_bpf_dsq_insert.md). This kfunc returns a boolean where the original did not have a return value. Kernel developers intention is to replace the original kfunc with this implementation in :octicons-tag-24: v7.3 (4 releases after this change). Its recommended to use the SCX compatibility library [`compat.bpf.h`](../../ebpf-library/scx/index.md#compatbpfh) which provides the [`scx_bpf_dsq_insert`](../../ebpf-library/scx/scx_bpf_dsq_insert.md) function which abstracts this away.
...
`true` on successful insertion, `false` on failure. On the root scheduler, `false` return triggers scheduler abort and the caller doesn't need to check the return value.

**Signature**

<!-- [KFUNC_DEF] -->
`#!c bool scx_bpf_dsq_insert___v2(struct task_struct *p, u64 dsq_id, u64 slice, u64 enq_flags)`
<!-- [/KFUNC_DEF] -->
