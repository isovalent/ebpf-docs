---
title: "KFunc 'bpf_xdp_pull_data'"
description: "This page documents the 'bpf_xdp_pull_data' eBPF kfunc, including its definition, usage, program types that can use it, and examples."
---
# KFunc `bpf_xdp_pull_data`

<!-- [FEATURE_TAG](bpf_xdp_pull_data) -->
[:octicons-tag-24: v6.18](https://github.com/torvalds/linux/commit/4dce1a0d7cf39575a5880414ea882890edd8d26f)
<!-- [/FEATURE_TAG] -->

Pull in non-linear xdp data.

## Definition

Pull in data in case the XDP buffer associated with `x` is non-linear and not all `len` are in the linear data area.

A call to this kfunc may reduce headroom. If there is not enough tail-room in the linear data area, metadata and data will be shifted down.

A call to this kfunc is susceptible to change the buffer geometry. Therefore, at load time, all checks on pointers previously done by the verifier are invalidated and must be performed again, if the kfunc is used in combination with direct packet access.
...
<!-- [KFUNC_DEF] -->
`#!c int bpf_xdp_pull_data(struct xdp_md *x, u32 len)`
<!-- [/KFUNC_DEF] -->

## Usage

Direct packet access allows reading and writing linear XDP data through packet pointers (for example, `&xdp_md->data + offsets`). The amount of data which ends up in the linear part of the `xdp_buff` depends on the NIC and its configuration. When a [`frag-capable XDP program`](../program-type/BPF_PROG_TYPE_XDP.md#xdp-fragments) wants to directly access headers that may be in the non-linear area, call this kfunc to make sure the data is available in the linear area. Alternatively, use [`bpf_dynptr_from_xdp`](bpf_dynptr_from_xdp.md), or [`bpf_xdp_load_bytes`](../helper-function/bpf_xdp_load_bytes.md) / [`bpf_xdp_store_bytes`](../helper-function/bpf_xdp_store_bytes.md) to access data without pulling.

This kfunc can also be used with [`bpf_xdp_adjust_head`](../helper-function/bpf_xdp_adjust_head.md) to decapsulate headers in the non-linear data area.
...
Example from [`tools/testing/selftests/net/lib/xdp_native.bpf.c`](https://github.com/torvalds/linux/blob/b927546677c876e26eba308550207c2ddf812a43/tools/testing/selftests/net/lib/xdp_native.bpf.c)
